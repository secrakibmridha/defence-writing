...
Translating these laboratory findings into actionable engineering practice is imperative for constructing resilient road infrastructure in Bangladesh. First, the Roads and Highways Department (RHD) must revise existing material specifications to enforce rigorous rheological and compositional quality control on imported bitumen, preventing the influx of substandard binders that compromise pavement integrity. Second, quarrying and crushing guidelines should mandate strict aggregate shape sorting. Introducing a specification for a minimum threshold (e.g., $20\%$) of cubical coarse aggregates will fundamentally enhance the interlocking matrix of the asphalt concrete. Implementing these two policy shifts will substantially increase the structural layer coefficient of the surfacing course, thereby reducing the requisite pavement thickness and delivering significant lifecycle cost savings for national highway projects.

\subsection{Future Work}
While this study establishes a foundational understanding of binder source and aggregate morphology interactions, several critical avenues remain for subsequent research to holistically validate these findings under simulated field conditions:
\begin{itemize}
    \item \textbf{Advanced Mechanical Characterisation:} Conducting Indirect Tensile Strength (ITS) testing (ASTM D6931) and Resilient Modulus ($M_R$) evaluations to quantify the fracture mechanics and dynamic load-bearing capacity of the optimised bituminous mixtures.
    \item \textbf{Moisture Susceptibility Profiling:} Given Bangladesh's aggressive subtropical monsoon climate, performing Tensile Strength Ratio (TSR) and Retained Marshall Stability tests is critical to ascertain the long-term stripping resistance and hydrothermal durability of the local bitumen-cubical aggregate matrix.
    \item \textbf{Accelerated Pavement Testing:} Executing dynamic wheel tracking and flexural fatigue tests to evaluate the empirical rutting depth progression and fatigue cracking resistance of the proposed mixes under cyclic vehicular loading scenarios.
    \item \textbf{Microstructural Analysis:} Utilising Scanning Electron Microscopy (SEM) and Fourier Transform Infrared (FTIR) spectroscopy to investigate the chemical bonding and microstructural interface between ERL bitumen and the cubical aggregate surface.
\end{itemize}
