...
Based on these findings, it is recommended that the Roads and Highways Department (RHD) specifications enforce strict quality control on the source and grading of imported bitumen binders. Furthermore, road construction guidelines in Bangladesh should encourage shape sorting of coarse aggregates to ensure a minimum proportion of cubical particles.

To address the exorbitant infrastructure costs in Bangladesh, a dual-level strategic roadmap for road construction is proposed:
\begin{enumerate}
    \item \textbf{Foundation Level (Subgrade Stabilization):} Prioritise the chemical stabilisation of weak local subgrade soils using cost-effective industrial by-products (e.g., lime or fly ash) to substantially increase the California Bearing Ratio (CBR) and reduce the required thickness of overlying layers.
    \item \textbf{Surface Level (Optimized Bituminous Mix):} Employ the synergistic combination of locally produced ERL bitumen and shape-optimised (minimum 20\% cubical) aggregates to achieve maximum Marshall stability. This yields a higher structural layer coefficient ($a_1$), thereby permitting a thinner, yet highly durable, asphalt surface course.
\end{enumerate}
This integrated approach fundamentally reduces the total volume of imported materials required, presenting a highly cost-effective and structurally resilient solution for Bangladesh's road network.
